...
const GetSongImgElement = () => document.getElementsByClassName('svelte-18bi8gg lcd lcd__music')[0].shadowRoot.querySelectorAll('.lcd__artwork-img')[0].src;
const GetSongElapsedTime = () => document.getElementsByClassName('svelte-18bi8gg lcd lcd__music')[0].shadowRoot.querySelectorAll('time.lcd-progress__time:nth-child(1)')[0].innerText;
const GetSongRemainingTime = () => document.getElementsByClassName('svelte-18bi8gg lcd lcd__music')[0].shadowRoot.querySelectorAll('time.lcd-progress__time:nth-child(2)')[0].innerText;
const GetTitle = () => document.getElementsByClassName('svelte-18bi8gg lcd lcd__music')[0].shadowRoot.querySelectorAll('.lcd-meta__primary > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > span:nth-child(1) > span:nth-child(1)')[0].innerText;
const GetAuther = () => document.getElementsByClassName('svelte-18bi8gg lcd lcd__music')[0].shadowRoot.querySelectorAll('.lcd-meta__secondary > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > span:nth-child(1)')[0].innerText.split('\n')[0];
const info = {
    video: {
        creator: '',
        title: '',
...
setInterval(() => {
    ...
    const timeP = (elapsedTime / totalTime) * 100;
    info.time = {
...
window.addEventListener('unload', (_e) => {
    ready = false;
    observer.disconnect();
    chrome.runtime.sendMessage({ type: 'unload', data: { service: info.extra.service }, uuid: info.extra.uuid }, async (response) => {
        console.log('[WatchRPC] [Content Script] received: ', response);
    });
});
